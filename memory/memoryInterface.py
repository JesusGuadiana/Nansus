# -----------------------------------------------------------------------------
#  Juan Fernando and Jesus’ Programming Language Memory Interface
...
class MemInterface():
    ...
    def temporary_memory_assign(self, v_type, value = None):
        return self.temp_memory.obtain_address(v_type, value)

    #Stores the value that is provided (defaulted to None by type) and returns memory allocation
    #No constant arrays or temporary arrays either.
    def sequential_global_assign(self, v_type, num_addresses, value = None):
        return self.global_memory.obtain_sequential_addresses(value_type, num_addresses, value)
    ...
    def type_of_memory(self, address):
        if (address >= self.global_memory.initial and address <= self.global_memory.final):
            ...
            return 'local'
        elif (address >= self.temp_memory.initial and address <= self.temporary_memory.final):
            return 'temporary'
    ...
    def edit_memory_content(self, address, content):
        ...
        if memory_type == 'global':
            self.global_memory.modify_content(address, content)
        elif memory_type == 'local':
            self.local_memory.modify_content(address, content)
        elif memory_type == 'temporary':
            self.temporary_memory.modify_content(address, content)
        elif memory_type == 'constant':
            self.constant_memory.modify_content(address, content)
