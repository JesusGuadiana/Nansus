...
class FuncDirectory():
	...
	def set_local_variable_dimension_two(self, identifier, v_type, v_identifier, v_address = 0):
		current_function = self.get_function(identifier)
		if current_function is not None:
			if current_function['local_variables'].variable_exists(v_identifier):
	...
	def get_quad_number(self, identifier):
		...
		if new_function is not None:  
			...
			#Function Address Getter for specified function
	def get_return_address(self, identifier):
		current_function = self.get_function(identifier)
		if current_function is not None:
			return current_function['return_address']
		else:
			print ("Function " + identifier + " does not exist.")
			sys.exit()
